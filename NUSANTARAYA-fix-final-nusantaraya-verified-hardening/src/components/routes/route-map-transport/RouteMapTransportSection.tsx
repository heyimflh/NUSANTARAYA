...
export function RouteMapTransportSection({
  ...
  status,
  activeRouteKey: _activeRouteKey,
  externalDaySelection,
  onViewInItinerary,
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      setSelection(sel);
      const mapSection = document.getElementById(ROUTE_SECTION_IDS.map);
      if (mapSection) scrollElementIntoView(mapSection, { block: "start" });

      const stop = mapModel.stops.find((item) => item.id === externalDaySelection.stopId);
