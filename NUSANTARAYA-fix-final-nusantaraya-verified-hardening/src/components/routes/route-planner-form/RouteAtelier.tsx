...
export function RouteAtelier() {
  ...
  const handleApplyRaniDraft = useCallback((draft: RouteAdjustmentDraft) => {
    ...
    const canonicalResolution = activeItinerary ? null : resolveRouteItinerary(result);
    const currentItinerary = activeItinerary ??
      (canonicalResolution?.status === "ready" ? canonicalResolution.itinerary : null);

    if (!currentItinerary) {
      announcer.announce("Itinerary aktif belum siap untuk disesuaikan.", "assertive");
      return;
    }
    ...
    if (isDraftStale(draft, result.id, result.version, currentItinerary.version)) {
