...
export function RouteRecommendationResultSection({
  ...
  const [itineraryExternalDay, setItineraryExternalDay] = useState<{ routeKey: string | null; dayNumber: number } | null>(null);
  ...
  const handleViewInMap = useCallback((context: {
    ...
    setMapExternalSelection({ ...context, routeKey: workspace.activeRouteKey });
  }, []);

  const handleViewInItinerary = useCallback((dayNumber: number) => {
    setItineraryExternalDay({ routeKey: workspace.activeRouteKey, dayNumber });
  }, []);
      ...
      >
        ...
        <div className="w-full">
          <DayByDayItinerarySection
            ...
            externalActiveDay={itineraryExternalDay?.routeKey === workspace.activeRouteKey ? itineraryExternalDay.dayNumber : null}
          />
        </div>
        ...
        <div className="w-full">
          <RouteMapTransportSection
            key={workspace.activeRouteKey ?? "idle"}
            result={workspace.recommendation}
            mapModel={workspace.mapModel}
            transportOptions={workspace.transportOptions}
            status={workspace.status}
            activeRouteKey={workspace.activeRouteKey}
            externalDaySelection={mapExternalSelection?.routeKey === workspace.activeRouteKey ? mapExternalSelection : null}
