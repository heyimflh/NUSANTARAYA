...
export function RaniAdjustmentLane({ result, itinerary, values, locale, onApplyDraft, onUndoDraft, canUndo = false }: RaniAdjustmentLaneProps) {
        ...
        <div className="flex flex-wrap gap-2 relative z-10 mt-2">
          ...
          {canUndo && onUndoDraft && (
            <button
              type="button"
              onClick={onUndoDraft}
              className="min-h-11 px-4 py-2 bg-white border border-[#E8E0CE] text-[#2C3E50] text-sm font-medium rounded-full hover:border-[#D4AF37] focus-visible:outline-2 focus-visible:outline-[#D4AF37] focus-visible:outline-offset-2"
            >
              {locale === "en" ? "Undo last adjustment" : "Batalkan penyesuaian terakhir"}
            </button>
          )}
        </div>
      )}
