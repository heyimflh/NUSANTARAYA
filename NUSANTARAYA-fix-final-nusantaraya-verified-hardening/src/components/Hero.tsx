...
export default function Hero() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Aktifkan smart color hanya ketika navbar menyentuh section terang (melewati hero)
  ...
  return (
    <>
      {/* FIXED NAVBAR — Uses plain nav to avoid Framer Motion transform breaking fixed positioning */}
      <nav 
    ...
    <section className="relative w-full h-[100svh] min-h-[640px] md:min-h-[700px] lg:h-[100dvh] lg:min-h-0 overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full h-full flex flex-col justify-between hero-safe-padding px-4 py-5 sm:px-6 sm:py-6 md:px-8 md:py-8 lg:p-10 lg:px-16">
        {/* Spacer for removed navbar (navbar is now outside section) */}
        ...
        <div className="absolute top-[38%] md:top-[35%] lg:top-[40%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col items-center text-center w-full px-4 z-0 pointer-events-none">
          ...
          >
            <div className="flex flex-col items-center gap-2 lg:gap-3">
              <h1
                className="text-[3rem] min-[390px]:text-[3.5rem] sm:text-[5rem] md:text-[6rem] lg:text-[110px] leading-[1] tracking-wider drop-shadow-lg uppercase"
                style={{
              ...
              <p
                className="text-lg min-[390px]:text-xl sm:text-2xl md:text-3xl lg:text-[32px] leading-tight tracking-wide drop-shadow-md text-white/90"
                style={{
                  fontFamily: "var(--font-philosopher)",
                  fontWeight: 400,
                }}
        ...
        <div className="flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end w-full gap-4 md:gap-4 lg:gap-8 mt-auto z-10">
          ...
          <div className="w-full md:max-w-[280px] lg:max-w-[340px] text-white hidden md:block shrink-0">
            <AnimatePresence mode="wait">
              ...
              >
                <h3
                  className="text-xl lg:text-2xl font-medium mb-1 drop-shadow-md"
                  style={{ fontFamily: "var(--font-outfit)", color: "#ffffff" }}
                >
                  Jelajahi Indonesia Secara Interaktif
                </h3>
                <p
                  className="text-xs lg:text-sm mb-4 lg:mb-5 font-light tracking-wide"
                ...
                <div className="flex flex-wrap items-center gap-1.5 lg:gap-2 mb-4 lg:mb-5">
                  {["Peta Interaktif", "Rute Wisata", "Cerita Budaya"].map(
                    (chip) => (
                      <span
                        key={chip}
                        className="text-[9px] lg:text-[10px] font-medium bg-white/10 backdrop-blur-md border border-white/20 px-2.5 py-1 lg:px-3 lg:py-1.5 rounded-full"
                        style={{ color: "#ffffff" }}
                      >
                        {chip}
                      </span>
                ...
                <p
                  className="text-xs lg:text-sm leading-relaxed font-light mb-6 lg:mb-8 drop-shadow-sm"
                  style={{ color: "rgba(255,255,255,0.9)" }}
                >
                  Jelajahi keindahan budaya, sejarah, alam, dan kuliner
                  Indonesia dalam satu sentuhan digital interaktif.
                </p>
            ...
            {/* Pagination */}
            <div
              className="mt-8 lg:mt-12 flex items-baseline gap-1"
          ...
          <div className="md:hidden pointer-events-auto flex flex-col w-full mb-2 z-20">
            <AnimatePresence mode="wait">
              ...
              >
                <span
                  className="text-[10px] sm:text-xs font-medium uppercase tracking-widest mb-1"
                  style={{ color: "rgba(255, 255, 255, 0.8)" }}
                >
                ...
                <p
                  className="text-xs sm:text-sm line-clamp-2 max-w-[90%] sm:max-w-[400px]"
                  style={{
            ...
            <div className="mt-4 flex items-center gap-3">
              <button
                className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-5 py-2.5 rounded-full text-xs font-medium active:scale-95 transition-transform shadow-lg"
                aria-label="Eksplorasi Peta"
              >
                Eksplorasi Peta
              </button>
            </div>
          ...
          <div className="flex flex-col items-start lg:items-end gap-4 lg:gap-6 w-full md:w-auto overflow-hidden">
            ...
            <div className="hidden md:flex items-end gap-3 lg:gap-4 overflow-x-auto pb-2 hide-scrollbar w-full max-w-full">
              ...
              <div className="flex flex-col gap-2 sm:gap-3 min-w-[min(76vw,260px)] sm:min-w-[280px] lg:min-w-[280px]">
                ...
                <div
                  className="flex justify-between items-start gap-2 sm:gap-3 drop-shadow-md"
                  style={{ color: "#ffffff" }}
                >
                  <div className="flex-1 min-w-0">
                    <h4
                      className="font-semibold text-xs sm:text-sm lg:text-sm mb-0.5 sm:mb-1 truncate"
                      style={{
                        fontFamily: "var(--font-outfit)",
                        color: "#ffffff",
                      }}
                    ...
                    <p
                      className="text-[10px] sm:text-[11px] leading-tight max-w-[170px] sm:max-w-[200px] line-clamp-2"
                      style={{ color: "rgba(255,255,255,0.8)" }}
                    >
                      {currentSlide.desc}
                    </p>
                  ...
                  <span
                    className="text-[10px] sm:text-xs font-medium bg-white/20 backdrop-blur-md border border-white/20 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md shrink-0 mt-0.5"
                    style={{ color: "#ffffff" }}
                  >
                    {currentSlide.subtitle}
                  </span>
              ...
              <div
                ...
                onClick={() =>
                  handleManualSlide((currentIndex + 1) % CAROUSEL_DATA.length)
                }
              ...
              <div
                ...
                onClick={() =>
                  handleManualSlide((currentIndex + 2) % CAROUSEL_DATA.length)
                }
