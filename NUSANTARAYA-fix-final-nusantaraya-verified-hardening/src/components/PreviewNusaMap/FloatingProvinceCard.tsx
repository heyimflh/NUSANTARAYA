...
export default function FloatingProvinceCard({
  province,
  isZoomed,
  onClose,
      ...
      <AnimatePresence mode="wait">
        {province && (
          ...
          >
            ...
            <div
              className={`absolute inset-0 bg-gradient-to-r from-[#D8B56D]/10 to-transparent pointer-events-none ${isZoomed ? "rounded-3xl" : "rounded-full"}`}
            ></div>

            {/* Back/Close Button (Side Tab on Desktop, Top-Right on Mobile) */}
            {isZoomed && onClose && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="absolute md:top-1/2 md:-left-[32px] md:-translate-y-1/2 md:w-[32px] md:h-16 top-3 right-3 w-8 h-8 flex items-center justify-center md:rounded-l-xl rounded-full bg-white/40 md:bg-white/40 backdrop-blur-[40px] md:hover:bg-white/60 hover:bg-white/80 border border-white/60 md:border-white/40 md:border-r-0 transition-all z-20 md:z-[-1] text-[#10233F] shadow-md md:shadow-[-10px_0_15px_-5px_rgba(0,0,0,0.1)] group cursor-pointer"
              >
                <svg
                  ...
                  className="transform transition-transform group-hover:-translate-x-0.5 hidden md:block"
                >
                ...
                <svg
                  ...
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="md:hidden block transform transition-transform group-hover:scale-110"
                >
                  <path d="M18 6 6 18" />
                  <path d="m6 6 12 12" />
                </svg>
                ...
                <div className="flex flex-row md:flex-col gap-3 md:gap-0 items-center md:items-stretch">
                  ...
                  >
                    ...
                    <div className="absolute bottom-3 left-4 right-4 hidden md:block pointer-events-none">
                      ...
                      <h4 className="font-serif font-bold text-2xl leading-tight drop-shadow-md" style={{ color: '#ffffff' }}>
                        {province.name}
                      </h4>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col justify-center md:hidden pr-6">
                    <p className="text-[8px] font-bold text-[#D8B56D] uppercase tracking-[0.2em] mb-0.5">
                      {province.region}
                    </p>
                ...
                <div className="px-1 md:px-2 pt-1 pb-1 md:pb-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 md:gap-2 mt-2 md:mt-4">
                    {province.highlights.map((h) => (
                      <span
                        key={h}
                        className="px-2 py-1 md:px-3 md:py-1.5 rounded-lg md:rounded-xl bg-white/50 border border-white/40 text-[9px] md:text-[10px] font-bold text-[#10233F] shadow-sm"
                      >
                        {h}
                      </span>
                    ))}
                  ...
                  <a
                    href={province.href}
                    className="mt-3 md:mt-5 w-full py-2.5 md:py-3.5 bg-black/80 hover:bg-black text-white text-[10px] md:text-xs text-center rounded-xl md:rounded-[1.25rem] font-bold tracking-wide transition-all duration-300 shadow-md flex justify-center items-center gap-2 group"
                  >
                    Eksplorasi Provinsi
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      ...
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      className="transform transition-transform group-hover:translate-x-1 md:w-3.5 md:h-3.5"
                    >
              ...
              <>
                ...
                <div className="flex flex-col justify-center py-1 relative z-10 pr-2">
                  ...
                  <p className="text-[9px] md:text-[10px] text-gray-600 font-bold uppercase tracking-widest mt-1 flex items-center gap-1.5">
                    ...
                    <span className="line-clamp-1 max-w-[120px] md:max-w-[150px]">
                      {province.highlights.join(" · ")}
                    </span>
                  </p>
                </div>
