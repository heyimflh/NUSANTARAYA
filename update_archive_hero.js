...
const tsPath = path.resolve('src/components/archive/ArchiveHeroSection.tsx');

const content = `"use client";

import { motion } from "framer-motion";
import { ArrowRight, ArrowDown, Library } from "lucide-react";
  ...
  return (
    <section className="relative w-full h-screen min-h-[750px] flex items-center overflow-hidden [--wave-h:80px] md:[--wave-h:120px] lg:[--wave-h:160px]">
        ...
        >
          ...
          <div className="absolute inset-0 z-10 bg-gradient-to-r from-[#F8F4EA] via-[#F8F4EA]/90 to-transparent w-full md:w-[60%] lg:w-[55%]" />
          
          {/* Additional gradient for bottom fade for seamless transition */}
          <div className="absolute inset-0 z-10 bg-gradient-to-t from-[#F8F4EA]/50 via-transparent to-transparent h-[30%] mt-auto" />
        </div>
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-20 relative z-20 h-full flex flex-col justify-center">
          <div className="max-w-[500px] mt-12 lg:mt-16">
            ...
            >
              {language === 'id' ? (
                <>Rumah<br />Ingatan yang<br />Hidup</>
              ) : (
                <>The Living<br />Memory House</>
              )}
            </motion.h1>
            ...
            >
              <button
                onClick={() => document.getElementById('archive-results')?.scrollIntoView({ behavior: 'smooth' })}
              ...
              <button
                onClick={() => document.getElementById('curator-selection')?.scrollIntoView({ behavior: 'smooth' })}
              ...
              >
                <div className="w-10 h-10 rounded-full border border-[#29221B]/20 flex items-center justify-center group-hover:bg-[#29221B] group-hover:text-white transition-colors">
                  <Library className="w-[16px] h-[16px]" />
                </div>
                {language === 'id' ? "Koleksi Kurator" : "Curator's Collection"}
              </button>
            </motion.div>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 w-full z-20 pointer-events-none h-[80px] md:h-[120px] lg:h-[160px]">
          <motion.div
            ...
            className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center justify-center pointer-events-auto cursor-pointer"
            onClick={() => document.getElementById('archive-results')?.scrollIntoView({ behavior: 'smooth' })}
          >
            <span className="font-inter text-[#29221B]/50 text-[11px] md:text-xs tracking-wide mb-2">
              {language === 'id' ? "Scroll untuk menjelajahi" : "Scroll to explore"}
            </span>
            <motion.div
              animate={{ y: [0, 5, 0] }}
...
console.log('Fixed ArchiveHeroSection.tsx');
