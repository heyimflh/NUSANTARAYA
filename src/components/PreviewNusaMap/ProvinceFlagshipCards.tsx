...
export default function ProvinceFlagshipCards({
  provinces,
}: ProvinceFlagshipCardsProps) {
    ...
    >
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden">
        ...
        <div className="absolute inset-0 w-full h-full z-0 pointer-events-none video-mask">
          ...
          >
            <source
              src="/assets/map/video_optimized-flagship.webm"
              type="video/webm"
            />
          </video>
          {/* Teal base overlay for clear video but good contrast - Removed expensive mix-blend-multiply */}
          <div className="absolute inset-0 bg-[#0D9488]/10 pointer-events-none"></div>
          {/* Gradient to darken the right side where text sits, using deep teal */}
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#062B35]/70 to-[#062B35]/95 pointer-events-none"></div>
          {/* Gradient to frame top and bottom */}
          <div className="absolute inset-0 bg-gradient-to-b from-[#062B35]/80 via-transparent to-[#062B35]/70 pointer-events-none"></div>
          {/* Bulletproof gap filler: covers any 1px sub-pixel rendering gap at the very top */}
          <div className="absolute top-0 left-0 w-full h-[2px] bg-[#fbf3e9] z-40 hidden md:block"></div>
          {/* Subtle cyan glow at the bottom */}
          <div className="absolute bottom-0 left-0 w-full h-1/3 bg-gradient-to-t from-[#22D3EE]/15 to-transparent pointer-events-none"></div>
        </div>
        ...
        <div className="absolute bottom-6 md:bottom-12 right-6 md:right-12 z-30 flex flex-col items-center gap-3 pointer-events-none opacity-70 hidden md:flex">
          <span
            className="text-[9px] font-extrabold tracking-[0.3em] text-[#67E8F9] uppercase"
        ...
        <div className="relative z-10 w-full max-w-[90rem] mx-auto px-5 md:px-12 flex flex-col h-full pt-16 sm:pt-20 md:pt-32 pb-6 sm:pb-12">
          ...
          <div className="flex-shrink-0 mb-4 md:mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-2 md:gap-4 relative z-20">
            ...
            <button className="hidden lg:flex items-center gap-3 text-sm font-bold text-[#67E8F9] hover:text-white transition-colors uppercase tracking-widest pb-2 border-b-2 border-[#67E8F9]/40 hover:border-white group">
              ...
              <svg
                className="w-5 h-5 transform transition-transform group-hover:translate-x-1.5"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2.5"
          ...
          <div className="flex flex-col lg:flex-row gap-4 md:gap-8 lg:gap-16 relative flex-1 min-h-0 items-center justify-between w-full pb-4 md:pb-8">
            ...
            <div className="hidden lg:block w-[45%] h-full max-h-[55vh] relative perspective-1000">
              ...
              <div className="absolute inset-0 rounded-3xl bg-[#062B35]/40 border border-[#22D3EE]/20 transform rotate-[-4deg] translate-y-4 scale-95 shadow-xl" />
              <div className="absolute inset-0 rounded-3xl bg-[#062B35]/60 border border-[#67E8F9]/30 transform rotate-[-2deg] translate-y-2 scale-[0.98] shadow-2xl" />
            ...
            />
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-8 md:p-10 pointer-events-none flex flex-col items-start transition-all duration-700 group-hover:opacity-0 group-hover:translate-y-4">
                      ...
                      >
                        ...
                        <h4
                          className="font-serif text-3xl md:text-5xl font-bold drop-shadow-md leading-tight"
                          style={{
                            color: "#67E8F9",
            ...
            <div className="w-full lg:w-[55%] flex flex-col h-full min-h-0 lg:min-h-[500px] max-h-full lg:max-h-[80vh] relative justify-start lg:justify-center pr-2 md:pr-4 overflow-hidden lg:overflow-visible pb-2 lg:pb-10 pt-2 lg:pt-0">
              <AnimatePresence mode="popLayout">
                {provinces
                  .slice(
                    activeIndex,
                    Math.min(activeIndex + 4, provinces.length),
                  )
                  .map((province, i) => {
                    const isFirst = i === 0;
            ...
            />
                        ...
                        <h4
                          className={`font-serif font-bold leading-tight transition-all duration-500 ${
                            isFirst
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1.5 md:gap-3 flex-wrap mb-4">
                                {province.highlights.map((highlight, idx) => (
                                  <div
                                    key={idx}
                                    className="flex items-center gap-1 md:gap-2 bg-[#22D3EE]/10 backdrop-blur-sm px-2.5 py-1 md:px-4 md:py-2 rounded-full border border-[#22D3EE]/30 hover:border-[#67E8F9]/60 transition-colors shadow-sm"
                                  >
                              ...
                              <div className="pt-1 pb-4 md:pt-2 md:pb-6">
                                ...
                                >
                                  ...
                                  <svg
                                    className="w-3.5 h-3.5 md:w-4 md:h-4 relative z-10 text-white transform transition-transform duration-500 ease-out group-hover/btn:translate-x-1.5"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    stroke="currentColor"
                                    strokeWidth="2.5"
