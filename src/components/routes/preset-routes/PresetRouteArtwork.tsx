...
export function PresetRouteArtwork({ config, alt = "", className = "" }: PresetRouteArtworkProps) {
  ...
  const secondary = config?.secondaryColor || "#0D1B2A";
  const patternType = config?.patternType || "geometric";

  // Generate SVG patterns based on type
  const renderPattern = () => {
    switch (patternType) {
      case "wave":
        return (
          <pattern id={`wave-${primary}`} x="0" y="0" width="100" height="20" patternUnits="userSpaceOnUse">
      ...
      case "topographic":
        return (
          <pattern id={`topo-${primary}`} x="0" y="0" width="100" height="100" patternUnits="userSpaceOnUse">
      ...
      case "batik":
        return (
          <pattern id={`batik-${primary}`} x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
      ...
      default:
        return (
          <pattern id={`geom-${primary}`} x="0" y="0" width="60" height="60" patternUnits="userSpaceOnUse">
  ...
  const patternId = `${patternType}-${primary}`;

  return (
    <div className={`relative w-full h-full overflow-hidden bg-[var(--planner-canvas)] ${className}`} role="img" aria-label={alt}>
      {/* Base gradient */}
      <div 
