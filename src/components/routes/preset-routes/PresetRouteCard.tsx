...
export function PresetRouteCard({ route, isActive, onViewRoute, onPrefill, className = "" }: PresetRouteCardProps) {
  ...
  return (
    <article 
      ...
      className={`group relative w-full bg-white/70 backdrop-blur-md rounded-[20px] overflow-hidden border ${isActive ? "border-[#C89A3D] ring-2 ring-[#C89A3D] ring-offset-2 ring-offset-[#F4EFE6]" : "border-[#E8E0CE]"} shadow-sm flex flex-col transition-all motion-reduce:transition-none hover:shadow-[0_8px_24px_rgba(42,36,31,0.06)] hover:-translate-y-1 motion-reduce:hover:translate-y-0 focus-within:ring-2 focus-within:ring-[#C89A3D] ${className}`}
    >
      {isActive && (
      ...
      {/* Visual Side */}
      <div className="w-full relative aspect-[4/3] sm:aspect-[3/2] overflow-hidden bg-[#F4EFE6]/50 border-b border-[#E8E0CE]">
      ...
      <div className="flex flex-col flex-1 p-5 sm:p-6">
        ...
        <div className="mt-auto flex flex-col xs:flex-row items-stretch xs:items-center gap-2 sm:gap-3 relative z-10">
          <button
            onClick={handleView}
            className="flex-1 px-4 py-2.5 bg-[#F4EFE6]/80 backdrop-blur-sm hover:bg-[#E8E0CE] text-[#2A241F] border border-[#E8E0CE] text-xs sm:text-sm font-bold rounded-full flex items-center justify-center gap-2 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[#C89A3D]"
